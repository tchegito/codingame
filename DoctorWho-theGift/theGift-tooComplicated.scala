    ...
    if (max < price)
        ...
        while (delta > 0) {
            ...
            while (doublonRemoved) {
                ...
                doublonRemoved = false
                for (i <- 0 until nbOods) {
                    val v = budgets(i)
                    if (valPresentes(v) > 1 && v > 0 && delta > 0) {
                        budgets(i) = v -1
                        valPresentes(v) = valPresentes(v) - 1
                        valPresentes(v-1) = valPresentes.getOrElse(v-1, 0) + 1
                        delta = delta - 1
                        doublonRemoved = true
                    }
                }
            ...
            if (delta == 0) {
                ...
                while (success) {
                    ...
                    val m = budgets.max
                    val min = budgets.min
                    val idxZero = budgets.indexOf(min)
